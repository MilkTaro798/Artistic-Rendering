...
void main() 
{
    ...
    vec3 worldTangent = normalize((normalMatrix * vec4(tangent, 0.0)).xyz);
    worldTangent = normalize(worldTangent - dot(worldTangent, worldNormal) * worldNormal);
    vec3 bitangent = cross(worldNormal, worldTangent);
    bitangent = normalize((normalMatrix * vec4(bitangent, 0.0)).xyz);
    mat3 tbn = transpose(mat3(worldTangent, bitangent, worldNormal));
